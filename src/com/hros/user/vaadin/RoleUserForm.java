...
public class RoleUserForm extends CustomComponent implements
	...
	private Set<UserInfo> selectedUserInfos;
	 ...
	 * 已授予左侧所选中的角色的用户
	 */
	List<UserInfo> assignedUserInfos;
	...
	public RoleUserForm() {
		...
		table_roles.addListener(new Table.ValueChangeListener() {
			public void valueChange(ValueChangeEvent event) {
				...
				// 将已授权的用户标记为选中状态
				assignedUserInfos = userService
						.selectUsersForRole2(selectedRoleInfo.getRoleID());
				table_users.setValue(assignedUserInfos);
			}
		});
		...
		table_users.addListener(new Table.ValueChangeListener() {
			public void valueChange(ValueChangeEvent event) {
				// in multiselect mode, a Set of itemIds is returned,
				// in singleselect mode the itemId is returned directly
				selectedUserInfos = (Set<UserInfo>) event.getProperty()
						.getValue();
			}
		});
		table_users.setSelectable(true);
		table_users.setMultiSelect(true);
	...
	public void buttonClick(ClickEvent event) {
		...
		if (event.getSource().equals(button_save_assigns)) {
			...
			//遍历,查找该为本角色删除的用户
			List<UserInfo> toDeleteUsers = new ArrayList<UserInfo>();
			for(UserInfo userInfo:assignedUserInfos){
				if(!selectedUserInfos.contains(userInfo)){
					toDeleteUsers.add(userInfo);
				}
			}
			userService.deleteUsersForRole(selectedRoleInfo.getRoleID(), toDeleteUsers);
			//遍历,查找该为本角色添加的用户
			List<UserInfo> toAddUsers = new ArrayList<UserInfo>();
			for(UserInfo userInfo:selectedUserInfos){
				if(!assignedUserInfos.contains(userInfo)){
					toAddUsers.add(userInfo);
				}
			}
			userService.addUsersForRole(selectedRoleInfo.getRoleID(), toAddUsers);
	...
	private VerticalLayout buildVerticalLayout_user() {
		...
		verticalLayout_user.addComponent(horizontalLayout_button);
